...
def catch_data(params):
	try:
		...
		response = requests.get('http://www.youtube-diff-checker.com:8000/playlists/', headers=params["headers"]).json()
		...
		for point in box:
			...
			response1 = requests.get('http://www.youtube-diff-checker.com:8000/n_playlist_music1/', params=params["params"], headers=params["headers"]).json()
			# print(response1[0])
...
def dl_register(params):
	...
	if params["dl_list"]:
		for i in params["dl_list"]:
			...
			response2 = requests.post('http://www.youtube-diff-checker.com:8000/d_playlist_music/', headers=params["headers"], json=json_data)
			...
			# print(params["headers"])
			response3 = requests.delete('http://www.youtube-diff-checker.com:8000/n_playlist_music/', params=d, headers=params["headers"])
...
def new_data(params):
	...
	for i in params["in_list"]:
		...
		response = requests.post('http://www.youtube-diff-checker.com:8000/musics/', headers=params["headers"], json=json_data)
		sleep(0.5)
	#-- for
	return 0
#--- EoF ---
