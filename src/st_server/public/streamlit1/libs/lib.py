...
def get_users_playlist(un):
    ...
    response1 = requests.post(
        'http://192.168.11.2:8000/user_playlists/userinfo',
        params=params,
    ...
    for i in response1:
        ...
        response2 = requests.get(
            'http://192.168.11.2:8000/playlist/original',
            params=params,
            headers=headers).json()
...
def get_delete_music(un):
    ...
    for i in response1:
        ...
        response2 = requests.get(
            'http://192.168.11.2:8000/d_playlist_music1/',
            params=params,
            headers=headers).json()
        for j in response2:
            headers = {
            ...
            response3 = requests.get(
                f'http://192.168.11.2:8000/musics/{music_original_id}',
                headers=headers).json()
            date = list(i.get("created_at"))[:17]
            date[4] = "年"
            date[7] = "月"
...
def register(un, url):
    ...
    response = requests.post(
        'http://192.168.11.2:8000/register/',
        params=params,
        headers=headers,
        json=json_data)
    return response
